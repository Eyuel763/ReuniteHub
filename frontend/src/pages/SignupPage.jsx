...
const SignupPage = () => {
  ...
  const [usernameStatus, setUsernameStatus] = useState({
    loading: false,
    available: false, // Changed from null to false
    message: '',
    suggestions: []
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://your-django-server/api/register/', {
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            {!usernameStatus.loading && usernameStatus.message && (
              <div className={`status ${usernameStatus.available ? 'success' : 'error'}`}>
                {usernameStatus.message}
                {usernameStatus.suggestions.length > 0 && (
                  <div className="suggestions">
                    Try: {usernameStatus.suggestions.slice(0, 3).map((s, i) => (
                      <span 
                        key={i} 
                        className="suggestion"
                        onClick={() => {
                          setFormData(prev => ({ ...prev, username: s }));
                          setUsernameStatus(prev => ({ ...prev, suggestions: [] }));
                        }}
                      >
                        {s}
                      </span>
          ...
          <div className="role-section">
            ...
            <div className="role-grid">
              ...
              <label className={`role-card ${formData.role === 'volunteer' ? 'active' : ''}`}>
                <input
                  type="radio"
                  name="role"
                  value="volunteer"
              ...
              <label className={`role-card ${formData.role === 'moderator' ? 'active' : ''}`}>
                <input
                  type="radio"
                  name="role"
                  value="moderator"
